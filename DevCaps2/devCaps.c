...
LRESULT CALLBACK WndProc(HWND hwnd,UINT message,WPARAM wParam,LPARAM lParam){
	...
	PAINTSTRUCT ps;
	TEXTMETRIC tm;
	static cxChar,cxCaps,cyChar,nCurrentDevice = IDM_SCREEN,nCurrentInfo = IDM_BASIC;
	HMENU hMenu;
	int count;
	static DWORD dwNeeded,dwReturned;
	static PRINTER_INFO_4 *pinfo4;
	DWORD i;
	static TCHAR szWindowText[64],szDevice[32];
	HANDLE hPrint;
	HDC hdcinfo;
	//0x00000024
	switch(message){
		...
		case WM_SETTINGCHANGE:
			...
			if(GetVersion() & 0x80000000){
				MessageBox(hwnd,"Windows 98","tip",0);
			}else{
				//MessageBox(hwnd,"Windows NT","tip",0);
		...
		case WM_COMMAND:
			...
			if(LOWORD(wParam) == IDM_SCREEN || LOWORD(wParam) < IDM_DEVMODE){
				//MessageBox(hwnd,"IDM_SCREEN","tip",0);
		...
		case WM_PAINT:
			...
			if(hdcinfo){
				switch(nCurrentInfo){
					case IDM_BASIC:
						//MessageBox(hwnd,"IDM_BASIC","tip",0);
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nShowCmd){
	...
	hwnd = CreateWindow(szAppName,TEXT("The DevCaps2 program"),WS_OVERLAPPEDWINDOW,CW_USEDEFAULT,CW_USEDEFAULT,CW_USEDEFAULT,CW_USEDEFAULT,NULL,NULL,hInstance,NULL);
